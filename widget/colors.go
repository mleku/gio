...
func (t *Colors) initDefaultPalette() {
	...
	t.palette.Neutral950 = hex("#0D1117")
...
// Initialize custom palette with provided accent colors
func (t *Colors) initCustomPalette(primary, secondary uint32) {
	// Generate palette from primary color
	t.generatePaletteFromColor(&t.palette.Primary50, &t.palette.Primary100, &t.palette.Primary200,
		&t.palette.Primary300, &t.palette.Primary400, &t.palette.Primary500, &t.palette.Primary600,
		&t.palette.Primary700, &t.palette.Primary800, &t.palette.Primary900, &t.palette.Primary950, primary)

	// Generate palette from secondary color
	t.generatePaletteFromColor(&t.palette.Secondary50, &t.palette.Secondary100, &t.palette.Secondary200,
		&t.palette.Secondary300, &t.palette.Secondary400, &t.palette.Secondary500, &t.palette.Secondary600,
		&t.palette.Secondary700, &t.palette.Secondary800, &t.palette.Secondary900, &t.palette.Secondary950, secondary)

	// Use default tertiary, error, and neutral palettes
	t.initDefaultPalette()
}

// Generate a complete palette from a single color
func (t *Colors) generatePaletteFromColor(c50, c100, c200, c300, c400, c500, c600, c700, c800, c900, c950 *color.NRGBA, baseColor uint32) {
	// Convert uint32 to NRGBA
	base := color.NRGBA{
		R: uint8(baseColor >> 16),
		G: uint8(baseColor >> 8),
		B: uint8(baseColor),
		A: 255,
	}
	...
	*c50 = t.lighten(base, 0.95)
	*c100 = t.lighten(base, 0.9)
	*c200 = t.lighten(base, 0.8)
	*c300 = t.lighten(base, 0.6)
	*c400 = t.lighten(base, 0.4)
	*c500 = base
	*c600 = t.darken(base, 0.2)
	*c700 = t.darken(base, 0.4)
	*c800 = t.darken(base, 0.6)
	*c900 = t.darken(base, 0.8)
	*c950 = t.darken(base, 0.9)
}

// Initialize color roles based on theme mode
...
func (t *Colors) initLightRoles() {
	...
	t.roles.OnPrimary = t.palette.Primary50
	...
	t.roles.OnSecondary = t.palette.Secondary50
	...
	t.roles.OnTertiary = t.palette.Tertiary50
	...
	t.roles.OnError = t.palette.Error50
	...
	t.roles.SurfaceVariant = t.palette.NeutralVariant200
	t.roles.OnSurfaceVariant = t.palette.NeutralVariant800

	// Outline roles
	t.roles.Outline = t.palette.NeutralVariant600
	...
	t.roles.InversePrimary = t.palette.Primary100
...
func (t *Colors) IsDark() bool {
	return t.themeMode == ThemeModeDark
}

func (t *Colors) IsLight() bool {
	return t.themeMode == ThemeModeLight
}

// Utility functions

// hex converts a hex color string to NRGBA
...
// lighten lightens a color by the given factor (0-1)
func (t *Colors) lighten(c color.NRGBA, factor float32) color.NRGBA {
	return color.NRGBA{
...
// darken darkens a color by the given factor (0-1)
func (t *Colors) darken(c color.NRGBA, factor float32) color.NRGBA {
	return color.NRGBA{
...
// applySurfaceTint applies a subtle surface tint to a color
func (t *Colors) applySurfaceTint(base color.NRGBA) color.NRGBA {
	// Blend the surface tint with the base color at 5% opacity
	return t.blendColors(base, t.surfaceTint, 0.05)
}

// blendColors blends two colors with the given opacity
func (t *Colors) blendColors(base, overlay color.NRGBA, opacity float32) color.NRGBA {
	return color.NRGBA{
		R: uint8(float32(base.R)*(1-opacity) + float32(overlay.R)*opacity),
		G: uint8(float32(base.G)*(1-opacity) + float32(overlay.G)*opacity),
		B: uint8(float32(base.B)*(1-opacity) + float32(overlay.B)*opacity),
		A: base.A,
	}
}

// rgbToHsv converts RGB to HSV
func (t *Colors) rgbToHsv(c color.NRGBA) (h, s, v float32) {
	r := float32(c.R) / 255
	g := float32(c.G) / 255
	b := float32(c.B) / 255
